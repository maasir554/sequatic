  ...
  return (
    <div className="flex flex-col items-center gap-2 text-center">
      <div
        ref={ref}
        className={cn(
          "z-10 flex flex-col gap-2 aspect-square items-center justify-center rounded-full border-2 border-blue-300 bg-white p-3 shadow-[0_0_20px_-12px_#155dfc]",
          className,
        )}
      >
        {children}
        {title && <p className="w-24 text-sm text-gray-600">{title}</p>}
      </div>
    </div>
  )
})
...
export function ArchitectureDiagram({ className }: { className?: string }) {
  ...
  return (
    ...
    <div
      className={cn(
        "relative flex h-[500px] w-full items-center justify-center overflow-hidden rounded-2xl border bg-gray-50/50 backdrop-blur-3xl p-10 overflow-x-auto",
        className,
      )}
      ref={containerRef}
      >
      <div className="flex h-full w-full flex-col items-center justify-between gap-4">
        ...
        <div className="flex w-full max-w-2xl flex-row items-center justify-around">
            ...
            <Circle ref={aiRef} title="Agentic AI">
                <BrainCircuit className="size-8 text-yellow-500" />
            </Circle>
            {/* Top: User */}
        ...
        <div className="flex w-full max-w-2xl flex-row items-center justify-around">
          ...
          <Circle ref={indexedDbRef} title="IndexedDB Snapshots">
            <Archive className="size-8 text-purple-500" />
          </Circle>
          <Circle ref={mongoDbRef} title="Cloud Backup">
            <Server className="size-8 text-gray-500" />
          </Circle>
        </div>
      </div>
